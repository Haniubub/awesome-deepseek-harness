---
title: "modsearch"
description: "Web plugin for DSH and the search bridge for every model without native web access."
keywords: "modsearch, search, plugin, deepseek harness, dsh"
---
# modsearch

> ⭐ 127 · ✅ active · plugin

## One-liner

Web plugin for DSH and the search bridge for every model without native web access.

## About

Something broken, or something missing? [Open an issue](https://github.com/liustack/modsearch/issues/new/choose). For everything else, come find me on X: **[@liustack](https://x.com/liustack)**. What you built with it, which harness you are on, what should come next. New releases land there first, and a proper community space is on the way.

## Author
**[liustack](https://github.com/liustack)**

## Links

- [GitHub Repository](https://github.com/liustack/modsearch)
- [Full README](https://github.com/liustack/modsearch#readme)
- [Back to the Plugins list](../plugins.md)
